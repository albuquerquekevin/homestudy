 PROCEDURE SP_P_AGENDAS_INSERT_MULTIPLO (
	...
	--CURSOR QUE FAZ O SLPIT DA STRING DE SUBTIPOS
  ...
  BEGIN
	...
	LOOP
		...
		IF PFLG_AUT = 1 THEN
			VAGE_DTA_PRE := F_AGENDA_AUTOMATICA(PAGE_DTA, VSUBTIPO.IDE_SUBTIPO);
			IF VAGE_DTA_PRE < TRUNC(CURRENT_DATE) THEN
				SELECT STA_DSC INTO VERROR FROM P_SUB_TP_AGENDAS WHERE STA_IDE = VSUBTIPO.IDE_SUBTIPO; -- TROCAR O PSTA_IDE PELO SUBTIPOID DO LOOP
		...
		,      0
		,      VCRE_IDE
		,      1, USER, SYSDATE, USER, SYSDATE, VSUBTIPO.IDE_SUBTIPO);
